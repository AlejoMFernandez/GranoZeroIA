...
function Header({ visible, soundOn, setSoundOn, ambientOn, setAmbientOn, onJumpCatalog, currentSection }) {
  ...
  return (
    <header className={`gz-header ${visible ? "is-visible" : ""} ${scrolled ? "is-scrolled" : ""}`}>
      ...
      <div className="gz-header-center">
        <span className="gz-header-line" />
        <span className="gz-header-tagline">PWA · aprender café</span>
      </div>
      ...
      <div className="gz-home-grid">
        <div className="gz-home-text">
          ...
          <div className="gz-cta-row">
            ...
            <button className="gz-cta gz-cta-ghost" onClick={() => {Sfx.click();document.getElementById('cata')?.scrollIntoView({behavior:'smooth'});}}>
              ¿Qué café sos?
            </button>
          </div>
...
function CenterpieceCup() {
  return (
    <div className="gz-stage">
      <div className="gz-stage-label">
        <span className="gz-stage-label-dot" />
        <span>Tu primera taza</span>
      </div>
      <div className="gz-stage-plinth" />
      <Steam count={5} height={140} color="rgba(241,234,216,.55)" delay={400} />
      ...
      <div className="gz-stage-deco">
        {[
        { x: -180, y: 60, r: -22, s: 56, op: .55 },
        { x: -140, y: 220, r: 18, s: 42, op: .4 },
        { x: 200, y: 240, r: -10, s: 48, op: .5 },
        { x: 220, y: 70, r: 40, s: 44, op: .4 },
        { x: 90, y: 300, r: 0, s: 36, op: .3 }].
        map((d, i) =>
      ...
      {/* Etiqueta callout que da contexto a la taza */}
      <div className="gz-stage-callout">
        <span className="gz-stage-callout-line" />
        <div>
  ...
  return (
    <section className="gz-catalog" ref={refEl} id="catalogo">
      <header className="gz-shead">
        <div className="gz-section-num">06 — Catálogo</div>
  ...
  return (
    <>
      ...
      <main className={`gz-main ${stage === "app" ? "is-ready" : ""}`}>
        ...
        <SectionCata onOpenCafe={setSelected} />
        <SectionGlosario />
        <SectionDiario />
        <Footer />
      </main>
...
function Footer() {
  const sections = [
    ...
    { id: "grano",    n: "02", label: "El Grano" },
    { id: "tueste",   n: "03", label: "Tueste" },
    { id: "molienda", n: "04", label: "Molienda" },
    { id: "metodos",  n: "05", label: "Métodos" },
    { id: "catalogo", n: "06", label: "Catálogo" },
    { id: "cata",     n: "07", label: "Cata" },
    { id: "glosario", n: "08", label: "Glosario" },
    { id: "diario",   n: "09", label: "Diario" },
  ];
  const jump = (id) => {
    Sfx.click();
  ...
  return (
    <footer className="gz-footer">
      ...
      <div className="gz-footer-grid">
        ...
        {/* Columna 3: colofón */}
        <div className="gz-footer-col">
          <span className="gz-footer-coltitle">Receta del sitio</span>
          <dl className="gz-footer-dl">
            <div><dt>Tipografías</dt><dd>Cormorant Garamond · DM Sans · Space Mono</dd></div>
            <div><dt>Paleta</dt><dd>#1F1410 · #C8881F · #F1EAD8</dd></div>
            <div><dt>Stack</dt><dd>React + JSX, Web Audio, SVG</dd></div>
            <div><dt>Tueste</dt><dd>Lento. Como debe ser.</dd></div>
          </dl>
        </div>
        ...
        <div className="gz-footer-col gz-footer-col-meta">
          ...
          <div className="gz-footer-meta-row">
            <span className="gz-footer-version">v0.1 · demo</span>
            <span className="gz-footer-year">{new Date().getFullYear()}</span>
          </div>
        </div>
      </div>
