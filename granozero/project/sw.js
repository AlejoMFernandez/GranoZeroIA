...
// Estrategia: network-first para archivos propios (siempre frescos), network-first para CDNs.

const CACHE_VERSION = "granozero-v2";
...
self.addEventListener("fetch", (event) => {
  ...
  if (isSameOrigin) {
    // Network-first para archivos propios: siempre la versión más nueva;
    // el caché sólo se usa si no hay conexión.
    event.respondWith(
      fetch(req).then((res) => {
